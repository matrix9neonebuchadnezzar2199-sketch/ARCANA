...
const aiSetDestination: ToolDefinition = { id: "ai_set_destination", name: "Set AI Destination", description: "Set NavMeshAgent destination point", descriptionJa: "NavMeshAgentの目的地を設定する", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiSetDestination", params); return { success: true, message: `Destination set for ${params.name}`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
...
const aiPatrol: ToolDefinition = { id: "ai_patrol", name: "Setup Patrol Route", description: "Create a patrol route with waypoints", descriptionJa: "ウェイポイント付きのパトロールルートを作成する", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiPatrol", params); return { success: true, message: `Patrol route set for ${params.name}`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
...
const aiChase: ToolDefinition = { id: "ai_chase", name: "Chase Target", description: "Make an AI agent chase a target object", descriptionJa: "AIエージェントにターゲットを追跡させる", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiChase", params); return { success: true, message: `${params.name} chasing ${params.targetName}`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
...
const aiFlee: ToolDefinition = { id: "ai_flee", name: "Flee From Target", description: "Make an AI agent flee from a target", descriptionJa: "AIエージェントにターゲットから逃走させる", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiFlee", params); return { success: true, message: `${params.name} fleeing from ${params.targetName}`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
...
const aiIdle: ToolDefinition = { id: "ai_idle", name: "Set AI Idle", description: "Stop the AI agent and enter idle state", descriptionJa: "AIエージェントを停止してアイドル状態にする", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiIdle", params); return { success: true, message: `${params.name} set to idle`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
...
const aiSetSpeed: ToolDefinition = { id: "ai_set_speed", name: "Set AI Speed", description: "Change the movement speed of an AI agent", descriptionJa: "AIエージェントの移動速度を変更する", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiSetSpeed", params); return { success: true, message: `Speed set to ${params.speed} on ${params.name}`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
...
const aiAvoidance: ToolDefinition = { id: "ai_avoidance", name: "Set Avoidance Priority", description: "Set the avoidance priority of an AI agent", descriptionJa: "AIエージェントの回避優先度を設定する", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiAvoidance", params); return { success: true, message: `Avoidance priority set to ${params.priority}`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
...
const aiVisualizePath: ToolDefinition = { id: "ai_visualize_path", name: "Visualize AI Path", description: "Draw the current navigation path of an agent", descriptionJa: "エージェントの現在のナビゲーション経路を描画する", category: "ai",
  ...
  handler: async (params) => { try { const r = await unityBridge.send("AiVisualizePath", params); return { success: true, message: `Path visualized for ${params.name}`, data: r }; } catch (e: any) { return { success: false, message: e.message }; } } };
